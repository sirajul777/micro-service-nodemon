...
def deduct_saldo(telegram_id, amount, note):
    """Atomically debit reseller balance only when enough funds remain."""
    ...
    try:
        ...
        affected = (
            s.query(BotReseller)
            .filter(BotReseller.id == row.id, BotReseller.saldo >= amount)
            .update({
...
def refund_saldo(telegram_id, amount, note):
    ...
    the voucher itself failed to provision on the router.
    """
    s = get_session()
    try:
        row = s.query(BotReseller).filter(BotReseller.telegramId == telegram_id).first()
