...
export class HotspotController {
  ...
  @Get(':session/hotspot/log') @RequirePermission('viewDashboard') async hotspotLog(@Param('session') session:string){const active=await this.mikrotik.listActiveHotspotUsers(session,'');if(!active.success)return[];return(active.users||[]).map((u:any)=>({id:u.id,user:u.user,address:u.address,uptime:u.uptime,bytesIn:u.bytes_in,bytesOut:u.bytes_out,time:new Date().toISOString()}));}
