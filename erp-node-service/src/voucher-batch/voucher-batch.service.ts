...
export class VoucherBatchService {
  ...
  private async toModel(e: VoucherBatchEntity): Promise<VoucherBatch> {
    return {
      id: e.id, profileName: e.profileName, profileColor: e.profileColor || '#1f6feb',
      price: e.price || 0, totalPrice: e.totalPrice || 0, validity: e.validity || '',
      caption: e.caption || '', sessionId: e.sessionId, nasName: e.nasName || '',
      createdBy: e.createdBy || '', createdAt: e.createdAt, resellerId: e.resellerId || '',
  ...
  async saveBatch(batch: VoucherBatch): Promise<VoucherBatch> {
    ...
    if (!entity) {
      entity = this.batchRepo.create({
        id: batch.id, sessionId: batch.sessionId, profileName: batch.profileName,
        profileColor: batch.profileColor || '#1f6feb', price: batch.price || 0,
        totalPrice: batch.totalPrice || 0, validity: batch.validity || '', caption: batch.caption || '',
        nasName: batch.nasName || '', createdBy: batch.createdBy || '',
        createdAt: batch.createdAt || new Date().toISOString(), resellerId: batch.resellerId || '',
        resellerName: batch.resellerName || '', vouchers: batch.vouchers || [],
      });
    } else {
      entity.profileName = batch.profileName; entity.profileColor = batch.profileColor || '#1f6feb';
      entity.price = batch.price || 0; entity.totalPrice = batch.totalPrice || 0;
      entity.validity = batch.validity || ''; entity.caption = batch.caption || '';
      entity.nasName = batch.nasName || ''; entity.createdBy = batch.createdBy || '';
      entity.resellerId = batch.resellerId || ''; entity.resellerName = batch.resellerName || '';
      entity.vouchers = batch.vouchers || [];
    }
    return this.toModel(await this.batchRepo.save(entity));
  }

  /** Atomically persists a new batch and its provisioning outbox event. */
  async createBatch(batch: VoucherBatch): Promise<VoucherBatch> {
    return this.batchCreation.create(batch);
  }
  ...
  async markUsed(sessionId: string, batchId: string, username: string, usedBy: string): Promise<boolean> {
    ...
    voucher.status = 'used'; voucher.usedBy = usedBy; voucher.usedAt = new Date().toISOString();
  ...
  getStats(batch: VoucherBatch) {
    ...
    return { total, used, remaining: total - used, usedPct: total > 0 ? Math.round((used / total) * 100) : 0 };
  }
  ...
  async readLocalProfileMeta(sessionId: string): Promise<Record<string, { profileColor?: string; caption?: string }>> {
    ...
    for (const [name, meta] of Object.entries(all)) result[name] = { profileColor: meta.profileColor, caption: meta.caption };
    return result;
  }
  ...
  async deleteWithRouter(sessionId: string, batchId: string, deleteMikrotik: boolean): Promise<{ success: boolean; deletedFromMikrotik: number; failedFromMikrotik: number; error?: string }> {
    ...
    let deletedFromMikrotik = 0; let failedFromMikrotik = 0;
    if (deleteMikrotik) {
      const test = await this.mikrotikGrpc.testConnect(sessionId);
      if (!test.success) return { success: false, deletedFromMikrotik: 0, failedFromMikrotik: 0, error: `Gagal konek ke router: ${test.error}` };
      for (const voucher of batch.vouchers) {
        ...
        if (res.success) deletedFromMikrotik++; else failedFromMikrotik++;
      }
    }
  ...
  async syncUsedFromMikrotik(sessionId: string): Promise<{ success: boolean; updated: number; message?: string }> {
    ...
    for (const batch of batches) {
      ...
      for (const voucher of batch.vouchers) {
        ...
        const routerUser = usersByName[voucher.username]; if (!routerUser) continue;
        ...
        if (hasDateComment || hasTraffic) {
          voucher.status = 'used'; voucher.usedBy = 'Hotspot'; voucher.usedAt = comment || new Date().toISOString();
