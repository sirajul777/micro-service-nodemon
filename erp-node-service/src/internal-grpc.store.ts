...
export class ErpSessionStore {
  ...
  async create(request: Record<string, any>) {
    ...
    const resp = await this.mikrotik.createSession({
      ...
      dnsName: String(request.dnsName || request.dns_name || ''),
      currency: String(request.currency || ''),
      reloadInterval: Number(request.reloadInterval ?? request.reload_interval) || 0,
      iface: String(request.iface || ''),
      idleTo: Number(request.idleTo ?? request.idle_to) || 0,
  ...
  async remove(id: string) {
    if (!String(id || '').trim()) return { success: false, error: 'id wajib diisi' };
    const resp = await this.mikrotik.deleteSession(String(id));
