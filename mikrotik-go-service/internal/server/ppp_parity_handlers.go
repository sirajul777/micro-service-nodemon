...
func (s *RouterServiceServer) ListPppSecrets(ctx context.Context, req *pb.ListPppSecretsRequest) (*pb.ListPppSecretsResponse, error) {
	...
	defer c.Close()
	args := []string{}
	if req.Profile != "" { args = append(args, "?profile="+req.Profile) }
...
func (s *RouterServiceServer) GetPppSecret(ctx context.Context, req *pb.GetPppSecretRequest) (*pb.GetPppSecretResponse, error) {
	...
	if err != nil { resp.Error = err.Error(); return resp, nil }
	if len(rows) == 0 { resp.Success = true; return resp, nil }
	r := rows[0]
	resp.Secret = &pb.PppSecret{Id:r[".id"],Name:r["name"],Password:r["password"],Service:r["service"],Profile:r["profile"],LocalAddress:r["local-address"],RemoteAddress:r["remote-address"],Comment:r["comment"],Disabled:r["disabled"]}
	resp.Success = true
	return resp, nil
}
