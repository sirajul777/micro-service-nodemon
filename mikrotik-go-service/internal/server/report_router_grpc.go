...
func (s *ReportRouterService) GetResumeReport(ctx context.Context, req *reportpb.GetResumeReportRequest) (*reportpb.GetResumeReportResponse, error) {
	...
	parts := strings.SplitN(monthId, " ", 2)
	_ = parts
	...
	for day := 1; day <= maxDay; day++ {
		dateKey := strings.ToLower(mm) + "/" + strings.TrimSpace(strconv.Itoa(day)) + "/" + strconv.Itoa(yearNum)
	...
	for _, row := range rows {
		...
		date := valueAt(nameParts, 0)
		vouchersDate := normalizeReportDate(date)
		entry, ok := daily[vouchersDate]
		if !ok {
			continue
		}
		entry.Vouchers++
	...
	for day := 1; day <= maxDay; day++ {
		dateKey := strings.ToLower(mm) + "/" + strings.TrimSpace(strconv.Itoa(day)) + "/" + strconv.Itoa(yearNum)
		resp.Daily = append(resp.Daily, daily[dateKey])
	}

	resp.TotalVouchers = int32(len(rows))
...
func normalizeReportDate(value string) string {
	...
	return strings.ToLower(parts[0]) + "/" + strings.TrimLeft(parts[1], "0") + "/" + parts[2]
}
