...
export class ReportResumeGrpcController {
  ...
  async resume(
    ...
    try {
      const response = await this.reportRouterGrpc.getResumeReport(
      ...
      if (!response?.success) {
        return res.status(502).json({
          success: false,
          message: response?.error || 'Report gRPC resume failed',
        });
      }
      ...
      return res.status(502).json({
        success: false,
        message: `Report gRPC unavailable: ${err?.message || err}`,
      });
    }
  }
}
