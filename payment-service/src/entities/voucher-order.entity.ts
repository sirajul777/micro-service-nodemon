 ...
 *
 * Key idea: each order gets a *unique payment amount* (e.g. Rp 10.237 where
 * 237 is the unique code), so when the PayHook Android app forwards a payment
 * notification (webhook), the server can match the incoming amount to this
 * order and auto-generate the voucher on the MikroTik router.
 */
...
export class VoucherOrderEntity {
  ...
  /** The unique 2-5 digit code appended to the price to make the amount matchable. */
  @Column({ type: 'int', default: 0 })
  uniqueCode: number;
  ...
  status: string; // pending | paid | failed | expired | manual | processing
