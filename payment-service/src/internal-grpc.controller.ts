...
export class InternalGrpcController {
  ...
  async listPayments(request: { status?: string }) {
    ...
    return {
      success: true,
      error: '',
      transactions: orders.map((order) => this.mapOrder(order)),
      total: orders.length,
    };
  }
  ...
  async createPaymentTest(request: { amount?: number; profile?: string }) {
    ...
    return {
      ...
      orderId: String(order.orderId),
      amount: Number(order.uniqueAmount || 0),
      qrString: String(order.qrString || ''),
      qrImage: String(order.qrImage || ''),
      status: String(order.status || ''),
    };
  }
  ...
  private mapConfig(config: any) {
    return {
      payhookStaticQris: String(config?.payhookStaticQris || ''),
      payhookUniqueDigits: Number(config?.payhookUniqueDigits || 0),
      payhookQrisExpiryMinutes: Number(config?.payhookQrisExpiryMinutes || 0),
      payhookExpiredRetentionDays: Number(config?.payhookExpiredRetentionDays || 0),
    };
  }
}
