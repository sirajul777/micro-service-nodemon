...
export const router = {
  ...
  addPppProfile: (session: string, body: Record<string, unknown>) => api(`/api/pppoe/${sessionPath(session)}/profiles`, json(body)),
  updatePppProfile: (session: string, name: string, body: Record<string, unknown>) => api(`/api/pppoe/${sessionPath(session)}/profiles/${encodeURIComponent(name)}`, { method: 'PUT', body: JSON.stringify(body) }),
  deletePppProfile: (session: string, name: string) => api(`/api/pppoe/${sessionPath(session)}/profiles/${encodeURIComponent(name)}`, { method: 'DELETE' }),
  pppActive: (session: string) => api(`/api/pppoe/${sessionPath(session)}/active`),
  disconnectPppActive: (session: string, name: string) => api(`/api/pppoe/${sessionPath(session)}/active/${encodeURIComponent(name)}`, json({})),
  pppSecrets: (session: string) => api(`/api/pppoe/${sessionPath(session)}/secrets`),
  addPppSecret: (session: string, body: Record<string, unknown>) => api(`/api/pppoe/${sessionPath(session)}/secrets`, json(body)),
  updatePppSecret: (session: string, name: string, body: Record<string, unknown>) => api(`/api/pppoe/${sessionPath(session)}/secrets/${encodeURIComponent(name)}`, { method: 'PUT', body: JSON.stringify(body) }),
  deletePppSecret: (session: string, name: string) => api(`/api/pppoe/${sessionPath(session)}/secrets/${encodeURIComponent(name)}`, { method: 'DELETE' }),
  enablePppSecret: (session: string, name: string) => api(`/api/pppoe/${sessionPath(session)}/secrets/${encodeURIComponent(name)}/enable`, json({})),
  disablePppSecret: (session: string, name: string) => api(`/api/pppoe/${sessionPath(session)}/secrets/${encodeURIComponent(name)}/disable`, json({})),
