...
export default function App() {
  ...
  const [modal, setModal] = useState<'user' | 'profile' | null>(null); const [editing, setEditing] = useState<Item | null>(null);
  ...
  return <div className="app-shell">
    ...
    <main><header><button className="icon mobile-only" onClick={() => setOpen(true)}><Menu size={20} /></button><div><span className="eyebrow">NETWORK OPERATIONS</span><h2>{title}</h2></div><div className="top-actions"><div className="search"><Search size={16} /><input placeholder="Search current page..." /></div><button className="icon" onClick={() => void load()}><RefreshCw size={17} className={loading ? 'spin' : ''} /></button></div></header>
      <section className="content">{error && <div className="error banner">{error}</div>}{page === 'dashboard' ? <Dashboard data={data} session={current} /> : page === 'qris' ? <QrisPage data={data} loading={loading} /> : page === 'live-report' ? <ReportPage data={data} loading={loading} /> : <TablePage page={page} data={data} loading={loading} session={session} onAdd={(kind) => { setEditing(null); setModal(kind); }} onEdit={(row, kind) => { setEditing(row); setModal(kind); }} onDelete={(row) => { if (page === 'hotspot-users' && window.confirm(`Delete user ${row.name}?`)) void runMutation(() => router.removeHotspotUser(session, String(row.name))); if (page === 'hotspot-profiles' && window.confirm(`Delete profile ${row.name}?`)) void runMutation(() => router.deleteHotspotProfile(session, String(row.name))); }} />}</section>
    </main>
...
function TablePage({ page, data, loading, session, onAdd, onEdit, onDelete }: any) {
  ...
  const crud = page === 'hotspot-users' || page === 'hotspot-profiles';
  return <div className="panel"><div className="panel-head"><div><h3>{nav.find((n) => n[1] === page)?.[0]}</h3><span>{rows.length} records</span></div><div className="panel-actions"><span className="badge">{loading ? 'LOADING' : 'LIVE'}</span>{crud && <button className="primary small" onClick={() => onAdd(page === 'hotspot-users' ? 'user' : 'profile')}><Plus size={14} /> Add</button>}</div></div><div className="table-wrap"><table><thead><tr>{columns.map((c) => <th key={c}>{humanize(c)}</th>)}{crud && <th>Actions</th>}</tr></thead><tbody>{rows.map((r, i) => <tr key={String(r.id || r.name || i)}>{columns.map((c) => <td key={c}>{formatCell(r[c])}</td>)}{crud && <td><div className="row-actions">{page === 'hotspot-profiles' && <button className="icon tiny" title="Edit" onClick={() => onEdit(r, 'profile')}>✎</button>}<button className="icon tiny danger" title="Delete" onClick={() => onDelete(r)}><Trash2 size={14} /></button></div></td>}</tr>)}</tbody></table>{!rows.length && <div className="empty">No records found.</div>}</div></div>;
}

function CrudModal({ kind, session, editing, onClose, onSaved }: { kind: 'user' | 'profile'; session: string; editing: Item | null; onClose: () => void; onSaved: () => void }) {
  const isUser = kind === 'user'; const [form, setForm] = useState<Record<string, string>>(() => isUser ? { name: editing?.name || '', password: '', profile: editing?.profile || '', comment: editing?.comment || '', limitUptime: editing?.limitUptime || '' } : { name: editing?.name || '', onLogin: editing?.onLogin || '', sessionTimeout: editing?.sessionTimeout || '', idleTimeout: editing?.idleTimeout || '', rateLimit: editing?.rateLimit || '', sharedUsers: editing?.sharedUsers || '', addressPool: editing?.addressPool || '' });
  const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const set = (key: string, value: string) => setForm((prev) => ({ ...prev, [key]: value }));
  const submit = async (e: React.FormEvent) => { e.preventDefault(); setBusy(true); setError(''); try {
    const result = isUser ? await router.addHotspotUser(session, form) : editing ? await router.updateHotspotProfile(session, editing.name, form) : await router.addHotspotProfile(session, form);
