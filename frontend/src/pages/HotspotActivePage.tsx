...
export default function HotspotActivePage({ session }: Props) {
  ...
  const disconnect = async (name: string) => {
    if (!window.confirm(`Disconnect hotspot user "${name}"?`)) return;
    setBusy(true);
    setNotice('');
    try {
      const result = await router.disconnectHotspotUser(session, name);
      if (result?.success === false) throw new Error(result.error || 'Disconnect failed.');
      await load();
      setNotice(`User ${name} disconnected.`);
    } catch (e: any) {
      setNotice(e?.message || 'Unable to disconnect hotspot user.');
      setBusy(false);
    }
  };
  ...
  return <div className="stack">
    ...
    <section className="panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>User</th><th>Address</th><th>MAC</th><th>Uptime</th><th>Profile</th><th>Session</th><th>Actions</th></tr></thead>
      <tbody>{filtered.map((row, i) => {
        const name = String(row.name || row.user || row.username || row.id || `row-${i}`);
        return <tr key={`${name}-${row.address || i}`}><td><b>{name}</b></td><td>{row.address || row.ip || '—'}</td><td>{row.macAddress || row.mac || '—'}</td><td>{row.uptime || '—'}</td><td>{row.profile || '—'}</td><td>{row.session || row.sessionId || '—'}</td><td><button className="icon tiny danger" title="Disconnect" disabled={busy || !name} onClick={() => void disconnect(name)}><WifiOff size={14}/></button></td></tr>;
