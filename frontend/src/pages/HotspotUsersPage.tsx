...
export default function HotspotUsersPage({ session }: Props) {
  ...
  return <div className="stack hotspot-users-page">
    <div className="hero"><div><span className="eyebrow">HOTSPOT MANAGEMENT</span><h3>Hotspot Users</h3><p>Manage RouterOS hotspot accounts for the active router.</p></div><div className="top-actions"><button className="button" onClick={() => void load()} disabled={busy}><RefreshCw size={15} className={busy ? 'spin' : ''}/> Refresh</button><button className="button primary" onClick={() => setShowForm(true)} disabled={busy}><Plus size={15}/> Add User</button></div></div>
    {notice && <div className="error banner">{notice}</div>}
    <section className="panel"><div className="panel-head"><div><h3><Users size={15}/> User Accounts</h3><span>{filtered.length} of {rows.length} users</span></div><span className="badge">{busy ? 'WORKING' : 'LIVE'}</span></div>
      <div className="table-controls hotspot-toolbar"><div className="table-search"><Search size={15}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search name, profile, comment..."/></div><div className="panel-actions"><span className="selection-count">{selected.length ? `${selected.length} selected` : 'Select users for bulk actions'}</span><button className="button secondary bulk-button" disabled={!selected.length || busy} onClick={() => void removeSelected()}><Trash2 size={14}/> Delete Selected ({selected.length})</button></div></div>
      <div className="table-wrap"><table className="user-table"><thead><tr><th style={{width:42}}><input type="checkbox" checked={allVisibleSelected} onChange={() => setSelected(allVisibleSelected ? [] : filtered.map(r => String(r.name)).filter(Boolean))}/></th><th>Name</th><th>Profile</th><th>Comment</th><th>Limit Uptime</th><th>Status</th><th>Actions</th></tr></thead><tbody>{filtered.map((r, i) => { const name = String(r.name || `row-${i}`); const disabled = String(r.disabled) === 'true' || r.disabled === true; return <tr key={name}><td><input type="checkbox" checked={selected.includes(name)} onChange={() => toggle(name)}/></td><td><b className="user-name">{name}</b></td><td><span className="profile-badge">{r.profile || 'default'}</span></td><td>{r.comment || '—'}</td><td>{r.limitUptime || '—'}</td><td><span className={disabled ? 'disabled-badge' : 'enabled-badge'}>{disabled ? 'Disabled' : 'Enabled'}</span></td><td><button className="icon tiny danger" title="Delete" disabled={busy} onClick={() => void removeOne(name)}><Trash2 size={14}/></button></td></tr>; })}</tbody></table>{!filtered.length && <div className="empty">No hotspot users found.</div>}</div>
    </section>
