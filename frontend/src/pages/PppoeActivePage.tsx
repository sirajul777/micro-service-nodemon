...
export default function PppoeActivePage({ session }: { session: string }) {
  ...
  return <div className="stack pppoe-active-page">
    <div className="hero">
    ...
    <section className="panel">
      <div className="panel-head"><div><h3>Active Sessions</h3><span className="session-count"><strong>{filtered.length}</strong> active session{filtered.length === 1 ? '' : 's'}</span></div><span className="badge">{busy ? 'WORKING' : 'LIVE'}</span></div>
      ...
      {filtered.map((r, i) => { const name = String(r.name || r.user || r.username || `row-${i}`); return <tr key={`${name}-${i}`}><td className="user-cell"><b>{name}</b></td><td className="address-cell">{r.address || r.remoteAddress || '—'}</td><td className="uptime-cell">{r.uptime || '—'}</td><td><span className="service-chip">{r.service || 'pppoe'}</span></td><td className="caller-cell">{r.callerId || r['caller-id'] || '—'}</td><td><button className="button disconnect-button" disabled={busy} onClick={() => void disconnect(name)}><WifiOff size={14}/> Disconnect</button></td></tr>; })}
