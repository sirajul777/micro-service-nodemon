    ...
    <div className="payment-page stack">
      <div className="page-hero">
        <div>
          ...
          <p>
            Transactions, configuration, status checks, and payment testing.
          </p>
        </div>
        <button
      ...
      <div className="stats">
        <Metric
          title="Transactions"
          value={stats.total ?? stats.totalTransactions ?? transactions.length}
        />
        <Metric title="Pending" value={stats.pending ?? 0} />
        <Metric title="Success" value={stats.success ?? stats.paid ?? 0} />
        <Metric
          title="Revenue"
          value={money(stats.revenue ?? stats.totalAmount ?? 0)}
        />
      </div>
      <div className="payment-grid">
              ...
              <tbody>
                {controls.visible.map((r, i) => (
                  <tr key={String(r.orderId || r.id || i)}>
                    <td>
                      <b>{formatCell(r.orderId || r.id)}</b>
                    </td>
                    <td>{money(r.amount)}</td>
                    <td>{formatCell(r.status)}</td>
                    <td>{formatCell(r.profile)}</td>
                    <td>{formatCell(r.createdAt)}</td>
                    <td>
                      <div className="row-actions">
                        <button
                          ...
                          disabled={busy}
                          onClick={() =>
                            void runCheck(String(r.orderId || r.id))
                          }
                        >
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              setMessage(
                                e?.message || "Unable to load detail.",
                              );
                            }
                          }}
                        >
        ...
        <section className="panel">
          <div className="panel-head">
            <div>
              <h3>
                <Settings2 size={15} /> Payment Configuration
              </h3>
              <span>Stored payment settings</span>
            </div>
            <button
              className="button primary"
              onClick={saveConfig}
          ...
          {!Object.keys(config).length && (
            <div className="empty">
              No payment configuration exposed by the backend.
            </div>
          )}
        </section>
      </div>
      <section className="panel">
        <div className="panel-head">
          <div>
            <h3>
              <TestTube2 size={15} /> Payment Test
            </h3>
            <span>Run the backend payment test flow.</span>
          </div>
        </div>
        <div className="form-grid compact">
          <label>
