...
const statusClass = (v: unknown) => `status-chip status-${normalize(v).replace(/\s+/g, "-")}`;
const statusLabel = (v: unknown) => {
  const value = normalize(v);
  if (value === "active") return "AKTIF";
  if (value === "suspended") return "DIBLOKIR";
  if (value === "unpaid") return "BELUM BAYAR";
...
export default function BillingPage() {
  ...
  const load = async () => {
    ...
    } catch (e: any) { setNotice(e?.message || "Gagal memuat data billing."); } finally { setBusy(false); }
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setSessions(rows);
        if (!readSessionFromDom()) setSession(rows[0]?.id || "");
      } catch (e: any) { setNotice(e?.message || "Gagal memuat router session."); }
    })();
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      ...
      const matchesType = tab !== "customers" || !type || normalize(r.type) === type;
      const matchesStatus = !status || normalize(r.status) === status;
  ...
  const saveCustomer = async () => {
    ...
    setBusy(true); setNotice("");
    try {
      await post(`${base}/customers`, { ...customer, price: Number(customer.price || 0), billDate: Number(customer.billDate || 1), graceDays: Number(customer.graceDays || 0) });
      setShowForm(false); resetCustomer(); await load(); setNotice("Pelanggan berhasil ditambahkan.");
  ...
  const action = async (label: string, fn: () => Promise<any>) => {
    setBusy(true); setNotice("");
    try { const r = await fn(); setNotice(r?.message || `${label} selesai.`); await load(); }
    catch (e: any) { setNotice(e?.message || `${label} gagal.`); }
  ...
  const importJson = async (file?: File) => {
    ...
    try {
      ...
      const rows = Array.isArray(parsed) ? parsed : Array.isArray(parsed?.customers) ? parsed.customers : [];
      let imported = 0;
      for (const row of rows) {
        if (!row?.name) continue;
        await post(`${base}/customers`, { ...row, type: row.type || row.service_type || "pppoe", mikrotikUser: row.mikrotikUser || row.pppoe_username || "", profile: row.profile || row.package || "", price: Number(row.price || row.monthlyPrice || 0), billDate: Number(row.billDate || row.bill_date || 1), graceDays: Number(row.graceDays || 3), autoDisable: row.autoDisable !== false });
        imported++;
      }
      await load(); setNotice(`${imported} pelanggan berhasil diimport.`);
    } catch (e: any) { setNotice(e?.message || "File JSON tidak valid atau import gagal."); }
    finally { setBusy(false); }
  };

  const unpaidCount = data.invoices.filter(r => ["unpaid", "overdue"].includes(normalize(r.status))).length;
  const suspendedCount = data.stats.suspended ?? data.customers.filter(r => normalize(r.status) === "suspended").length;
  const activeCount = data.stats.activeCustomers ?? data.customers.filter(r => normalize(r.status) === "active").length;

  return <div className="billing-page stack">
    <div className="billing-reference-head">
      <div><h3>Pelanggan Billing</h3><p>Kelola pelanggan internet dan status tagihan bulanan.</p></div>
      <button className="button" onClick={() => void load()} disabled={busy}><RefreshCw size={14} className={busy ? "spin" : ""} /> Refresh</button>
    </div>
    ...
    <div className="stats billing-stats">
      ...
      <Metric icon={<ShieldCheck size={17} />} title="Aktif" value={activeCount} kind="green" />
    ...
    {tab === "customers" ? <section className="panel billing-customer-panel">
      <div className="panel-head billing-customer-head">
        <h3><Users size={16} /> Daftar Pelanggan</h3>
        <div className="billing-toolbar">
          <select value={type} onChange={e => setType(e.target.value)}><option value="">Semua Tipe</option><option value="pppoe">PPPoE</option><option value="hotspot">Hotspot</option></select>
          <select value={status} onChange={e => setStatus(e.target.value)}><option value="">Semua Status</option><option value="active">Aktif</option><option value="suspended">Diblokir</option></select>
          <button className="button secondary" onClick={() => void runOverdue()} disabled={busy}><Ban size={13} /> Proses Overdue</button>
          <label className="button secondary billing-import-button"><Upload size={13} /> Import JSON<input type="file" accept="application/json,.json" onChange={e => void importJson(e.target.files?.[0])} /></label>
        </div>
      </div>
      ...
      <div className="table-wrap billing-reference-table">
        <table><thead><tr><th>Nama</th><th>Username MikroTik</th><th>Tipe</th><th>Paket</th><th>Tagihan/Bulan</th><th>Tgl Tagih</th><th>Status</th><th>Telegram</th><th>Aksi</th></tr></thead>
          <tbody>{filtered.map((r, i) => <tr key={r.id || i}>
            <td><b className="entity-cell">{cell(r.name)}</b>{r.phone && <small>{r.phone}</small>}</td>
            <td className="mono-cell">{cell(r.mikrotikUser)}</td>
            <td><span className="type-chip">{cell(r.type)}</span></td>
            <td>{cell(r.profile)}</td>
            <td className="money-cell billing-green-money">{money(r.price)}</td>
            <td>{r.billDate ? `Tgl ${cell(r.billDate)}` : "—"}</td>
            <td><span className={statusClass(r.status)}>{statusLabel(r.status)}</span></td>
            <td><span className="access-chip">{cell(r.telegramId)}</span></td>
            <td><div className="action-stack">{normalize(r.status) === "suspended" ? <button className="icon tiny" title="Aktifkan" disabled={busy} onClick={() => void action("Aktifkan", () => post(`${base}/customers/${esc(r.id)}/re-enable`))}><ShieldCheck size={13}/></button> : <button className="icon tiny danger" title="Blokir" disabled={busy} onClick={() => void action("Blokir", () => post(`${base}/customers/${esc(r.id)}/suspend`))}><Ban size={13}/></button>}</div></td>
          </tr>)}</tbody>
        </table>
        {!filtered.length && <div className="empty">Belum ada pelanggan billing.</div>}
      </div>
    </section> : <section className="panel">
      <div className="panel-head"><div><h3>Billing Workspace</h3><span>Invoices dan settlements</span></div><div className="panel-actions billing-tabs"><button className="button secondary" onClick={() => { setTab("customers"); setQuery(""); setStatus(""); }}>Customers</button><button className={tab === "invoices" ? "button primary" : "button secondary"} onClick={() => setTab("invoices")}>Invoices</button><button className={tab === "settlements" ? "button primary" : "button secondary"} onClick={() => setTab("settlements")}>Settlements</button></div></div>
      ...
      <div className="table-wrap"><table><thead><tr>{tab === "invoices" ? <><th>Customer</th><th>Period</th><th>Amount</th><th>Due Date</th><th>Status</th><th>Paid By</th><th>Actions</th></> : <><th>Collector</th><th>Amount</th><th>Status</th><th>Created</th><th>Verified</th></>}</tr></thead><tbody>{filtered.map((r, i) => tab === "invoices" ? <tr key={r.id || i}><td><b className="entity-cell">{cell(r.customerName)}</b></td><td>{cell(r.period)}</td><td className="money-cell">{money(r.amount)}</td><td>{cell(r.dueDate)}</td><td><span className={statusClass(r.status)}>{statusLabel(r.status)}</span></td><td>{cell(r.paidBy)}</td><td><div className="action-stack">{normalize(r.status) !== "paid" && <button className="icon tiny" title="Mark paid" disabled={busy} onClick={() => void action("Bayar invoice", () => post(`${base}/invoices/${esc(r.id)}/pay`, { paidBy: "Admin" }))}><CheckCircle2 size={13}/></button>}<button className="icon tiny" title="Kirim pengingat" disabled={busy} onClick={() => void action("Kirim pengingat", () => post(`${base}/invoices/${esc(r.id)}/send-reminder`))}><Send size={13}/></button></div></td></tr> : <tr key={r.id || i}><td><b className="entity-cell">{cell(r.collectorName)}</b></td><td className="money-cell">{money(r.amount)}</td><td><span className={statusClass(r.status)}>{statusLabel(r.status)}</span></td><td className="mono-cell">{cell(r.createdAt)}</td><td className="mono-cell">{cell(r.verifiedAt)}</td></tr>)}</tbody></table>{!filtered.length && <div className="empty">No records found.</div>}</div>
      {tab === "invoices" && <div className="section-actions"><button className="button primary" disabled={busy} onClick={() => void action("Generate monthly invoices", () => post(`${base}/invoices/generate`))}><FileText size={14}/> Generate Monthly Invoices</button><button className="button secondary" disabled={busy} onClick={() => void runOverdue()}>Run Overdue</button></div>}
