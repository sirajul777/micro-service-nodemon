...
import { CheckCircle2, FileText, RefreshCw, Send, ShieldAlert, ShieldCheck, Upload, Users, WalletCards, Ban, CircleDollarSign } from "lucide-react";
...
export default function BillingPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      ...
      const matchesType = tab !== "customers" || !type || String(r.type || "").toLowerCase() === type;
  ...
  const saveCustomer = async () => {
    ...
    catch (e: any) { setNotice(e?.message || "Gagal menambah pelanggan."); }
    finally { setBusy(false); }
  };
  ...
  return (
    <div className="billing-page stack">
      <div className="billing-reference-head">
        <div><span className="eyebrow">BILLING</span><h3>Pelanggan Billing</h3></div>
      ...
      {tab === "customers" ? (
        <section className="panel billing-customer-panel">
          ...
          <div className="table-wrap billing-reference-table">
            <table><thead><tr><th>Nama</th><th>Username MikroTik</th><th>Tipe</th><th>Paket</th><th>Tagihan/Bulan</th><th>Tgl Tagih</th><th>Status</th><th>Telegram</th><th>Aksi</th></tr></thead>
              <tbody>{filtered.map((r, i) => <tr key={r.id || i}>
                ...
                <td><span className={statusClass(r.status)}>{String(r.status || "—") === "active" ? "AKTIF" : cell(r.status)}</span></td>
                <td><span className="access-chip">{cell(r.telegramId)}</span></td>
                <td><div className="action-stack">{r.status === "suspended" ? <button className="icon tiny" title="Aktifkan" disabled={busy} onClick={() => void action("Aktifkan", () => post(`${base}/customers/${esc(r.id)}/re-enable`))}><ShieldCheck size={13}/></button> : <button className="icon tiny danger" title="Blokir" disabled={busy} onClick={() => void action("Blokir", () => post(`${base}/customers/${esc(r.id)}/suspend`))}><Ban size={13}/></button>}</div></td>
              </tr>)}</tbody></table>
...
function Metric({ icon, title, value, kind }: { icon: ReactNode; title: string; value: unknown; kind: string }) {
  return <div className={`stat billing-stat ${kind}`}><div className="stat-icon">{icon}</div><div><strong>{String(value)}</strong><span>{title}</span></div></div>;
}

function CustomerModal({ value, setValue, busy, onClose, onSave }: { value: Row; setValue: (v: Row) => void; busy: boolean; onClose: () => void; onSave: () => void }) {
  const field = (key: string, label: string, type = "text") => <label><span>{label}</span><input type={type} value={value[key] ?? ""} onChange={e => setValue({ ...value, [key]: e.target.value })} /></label>;
  return <div className="modal-backdrop"><div className="modal"><div className="modal-head"><div><span className="eyebrow">BILLING CUSTOMER</span><h3>Tambah Pelanggan</h3></div><button className="icon" onClick={onClose}>×</button></div><form onSubmit={e => { e.preventDefault(); onSave(); }}><div className="form-grid">{field("name", "Nama Pelanggan")}{field("mikrotikUser", "Username MikroTik")}{field("type", "Tipe")}{field("profile", "Paket / Profile")}{field("phone", "No. HP / WA")}{field("telegramId", "Telegram ID")}{field("address", "Alamat")}{field("price", "Harga Tagihan/Bulan", "number")}{field("billDate", "Tanggal Tagih", "number")}{field("graceDays", "Grace Period", "number")}{field("note", "Catatan")}</div><div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Batal</button><button className="button primary" disabled={busy}>Simpan</button></div></form></div></div>;
}

function SettlementForm({ base, onDone, busy, setBusy, setNotice }: { base: string; onDone: () => Promise<void> | void; busy: boolean; setBusy: (v: boolean) => void; setNotice: (v: string) => void }) {
  const [name, setName] = useState(""); const [amount, setAmount] = useState("0");
  return <div className="settlement-form"><input value={name} onChange={e => setName(e.target.value)} placeholder="Collector name"/><input type="number" min="0" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Amount"/><button className="button primary" disabled={busy || !name} onClick={async () => { setBusy(true); try { await post(`${base}/settlements/submit`, { collectorName: name, amount: Number(amount) }); setNotice("Settlement submitted."); setName(""); setAmount("0"); await onDone(); } catch (e: any) { setNotice(e?.message || "Settlement failed."); } finally { setBusy(false); } }}>Submit Settlement</button></div>;
}
