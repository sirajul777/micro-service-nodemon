...
export default function VoucherOperationsPage({ session }: Props) {
  ...
  return <div className="stack voucher-operations-page">
    ...
    <section className="panel"><div className="panel-head"><div><h3>{tab === 'batches' ? 'Voucher Batches' : 'Voucher Types'}</h3><span>{filtered.length} records</span></div><div className="panel-actions"><div className="voucher-tabs"><button className={tab === 'batches' ? 'button primary' : 'button secondary'} onClick={() => { setTab('batches'); setQuery(''); }}>Batches</button><button className={tab === 'types' ? 'button primary' : 'button secondary'} onClick={() => { setTab('types'); setQuery(''); }}>Types</button></div><div className="table-search"><Search size={14}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search vouchers..."/></div></div></div>
      <div className="table-wrap"><table><thead>{tab === 'batches' ? <tr><th>ID</th><th>Profile</th><th>Price</th><th>Validity</th><th>Status</th><th>Created</th><th>Stats</th><th>Actions</th></tr> : <tr><th>Name</th><th>Price</th><th>Duration</th><th>Profile</th><th>Enabled</th><th>Actions</th></tr>}</thead><tbody>{filtered.map((r, i) => tab === 'batches' ? <tr key={String(r.id || i)}><td><span className="record-id">{r.id || '—'}</span></td><td><span className="profile-cell">{r.profileName || r.profile || '—'}</span></td><td><span className="value-chip">{r.price ?? '—'}</span></td><td><span className="value-chip">{r.validity || '—'}</span></td><td><span className="status-chip">{r.status || (r.stats?.remaining != null ? 'active' : '—')}</span></td><td><span className="created-at">{r.createdAt || '—'}</span></td><td><span className="metric-chip">{r.stats ? `${r.stats.used ?? 0} used / ${r.stats.remaining ?? 0} left` : '—'}</span></td><td><div className="action-stack"><button className="icon tiny danger" disabled={busy} onClick={() => void deleteBatch(String(r.id))}><Trash2 size={14}/></button></div></td></tr> : <tr key={String(r.id || r.name || i)}><td><b>{r.name || '—'}</b></td><td><span className="value-chip">{r.price ?? '—'}</span></td><td><span className="value-chip">{r.duration || r.validity || '—'}</span></td><td>{r.profile || '—'}</td><td><span className={r.enabled === false || r.enabled === 'false' ? 'value-chip disabled-chip' : 'value-chip enabled-chip'}>{r.enabled === false || r.enabled === 'false' ? 'No' : 'Yes'}</span></td><td><div className="action-stack"><button className="button secondary" disabled={busy} onClick={async () => { try { setBusy(true); await voucher.toggleVoucherType(String(r.id)); await load(); } catch (e:any) { setNotice(e?.message || 'Unable to toggle type.'); setBusy(false); } }}>{r.enabled === false || r.enabled === 'false' ? 'Enable' : 'Disable'}</button><button className="icon tiny danger" disabled={busy} onClick={async () => { if (!window.confirm(`Delete voucher type \"${r.name}\"?`)) return; try { setBusy(true); await voucher.deleteVoucherType(String(r.id)); await load(); } catch (e:any) { setNotice(e?.message || 'Unable to delete voucher type.'); setBusy(false); } }}><Trash2 size={14}/></button></div></td></tr>)}</tbody></table>{!filtered.length && <div className="empty">No voucher records found.</div>}</div>
    </section>
