...
export default function SchedulerPage({ session }: Props) {
  ...
  const open = (row?: Row) => { setEditing(row || null); setForm({ ...empty, name: row?.name ?? row?.id ?? '', startDate: row?.startDate ?? row?.start_date ?? '', startTime: row?.startTime ?? row?.start_time ?? '', interval: row?.interval ?? '', onEvent: row?.onEvent ?? row?.on_event ?? '', disabled: row?.disabled === true || row?.disabled === 'true', comment: row?.comment ?? '' }); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const result = editing ? await router.updateScheduler(session, String(editing.name), { onEvent: body.onEvent, disabled: body.disabled, comment: body.comment }) : await router.addScheduler(session, body);
      if (result?.success === false) throw new Error(result.error || 'Operation failed.');
      const wasEditing = !!editing; close(); await load(); setNotice(wasEditing ? 'Scheduler updated.' : 'Scheduler created.');
    } catch (e: any) { setNotice(e?.message || 'Unable to save scheduler.'); setBusy(false); }
  };
  return <div className="stack scheduler-page">
    ...
    <section className="panel"><div className="panel-head"><div><h3><CalendarClock size={15}/> Scheduled Jobs</h3><span>{visible.length} of {rows.length} schedulers</span></div><span className="badge">{busy ? 'WORKING' : 'LIVE'}</span></div>
      <div className="table-controls"><div className="table-search scheduler-search"><Search size={15}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search name, event, comment..."/></div></div>
      <div className="table-wrap"><table><thead><tr><th>Name</th><th>Start Date</th><th>Start Time</th><th>Interval</th><th>On Event</th><th>Status</th><th>Comment</th><th>Actions</th></tr></thead><tbody>{visible.map((r,i) => { const name = String(r.name || r.id || i); const disabled = r.disabled === true || r.disabled === 'true'; return <tr key={name}><td><b>{name}</b></td><td><span className="schedule-date">{r.startDate || r.start_date || '—'}</span></td><td><span className="schedule-time">{r.startTime || r.start_time || '—'}</span></td><td><span className="schedule-interval">{r.interval || '—'}</span></td><td className="code-cell">{r.onEvent || r.on_event || '—'}</td><td><span className={`schedule-status${disabled ? ' is-disabled' : ''}`}><i/>{disabled ? 'DISABLED' : 'ENABLED'}</span></td><td>{r.comment || '—'}</td><td><div className="row-actions"><button className="button secondary tiny" disabled={busy} onClick={() => open(r)}>Edit</button></div></td></tr>; })}</tbody></table>{!visible.length && <div className="empty">No scheduler entries found.</div>}</div>
    </section>
...
function Modal({ form, setForm, editing, busy, close, save }: { form: any; setForm: any; editing: Row | null; busy: boolean; close: () => void; save: () => void }) {
  ...
  return <div className="modal-backdrop"><div className="modal scheduler-modal"><div className="modal-head"><div><span className="eyebrow">SCHEDULER</span><h3>{editing ? 'Edit Scheduler' : 'Add Scheduler'}</h3></div><button className="icon" onClick={close}><X size={18}/></button></div>
    <form onSubmit={e => { e.preventDefault(); save(); }}><div className="form-grid">
      {field('name', 'Name', '', !!editing)}{field('startDate', 'Start Date', 'Jan/01/2026')}{field('startTime', 'Start Time', '00:00:00')}{field('interval', 'Interval', '1d')}{field('onEvent', 'On Event', '/system script run ...', false, 'RouterOS command or script to execute.')}{field('comment', 'Comment')}
    ...
    </div><div className="modal-actions"><button type="button" className="button secondary" onClick={close}>Cancel</button><button className="button primary" disabled={busy}><Plus size={15}/>{editing ? 'Save Changes' : 'Create Scheduler'}</button></div></form></div></div>;
}
