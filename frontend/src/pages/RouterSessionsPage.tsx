...
const asForm=(s:Session):Form=>({...blank,id:String(s.id||''),name:String(s.name||''),ip:String(s.ip||''),port:String(s.port??8728),user:String(s.user||''),password:'',hotspotName:String(s.hotspotName||''),dnsName:String(s.dnsName||''),currency:String(s.currency||'Rp'),reloadInterval:String(s.reloadInterval??10),iface:String(s.iface||'ether1'),idleTo:String(s.idleTo??0),livereport:String(s.livereport||'enable')});
...
export default function RouterSessionsPage(){
  ...
  const save=async()=>{
    if(!form.id||!form.name||!form.ip||!form.user){setNotice('Session ID, router name, IP, and username are required.');return}
    setBusy(true);setNotice('');
    try{
      const payload:Record<string,unknown>={...form,port:Number(form.port)||8728,reloadInterval:Number(form.reloadInterval)||10,idleTo:Number(form.idleTo)||0};
  ...
  return <div className="stack">
    <div className="hero"><div><span className="eyebrow">SYSTEM</span><h3>Sessions</h3><p>Manage MikroTik router connections used by the control plane.</p></div><button className="primary small"onClick={openAdd}disabled={busy}><Plus size={15}/> Add Router</button></div>
