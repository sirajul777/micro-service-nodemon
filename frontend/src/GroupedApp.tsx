...
const existing = (
  ...
  disabled = false,
): ExistingTarget => ({ label, target, icon, requiresSession, disabled });
const route = (
  label: string,
  path: string,
  icon: any,
  requiresSession = false,
  disabled = false,
): RouteTarget => ({ label, path, icon, requiresSession, disabled });
const hasPath = (item: MenuItem): item is RouteTarget => "path" in item;

const groups: MenuGroup[] = [
  {
    label: "Utama",
    items: [existing("Dashboard", "Overview", Activity, true)],
  },
  {
    label: "Hotspot",
    items: [
      ...
      new Map(
        groups.flatMap((group) =>
          group.items.map((item) => [
            hasPath(item) ? item.path : item.target,
            item.label,
          ]),
        ),
      ),
  ...
  useEffect(() => {
    ...
    const attach = () => {
      ...
      const syncActive = () => {
        ...
        const activeButton = sidebar.querySelector<HTMLElement>(
          "nav:not(.grouped-nav) .nav.active",
        );
        const text = activeButton?.textContent?.trim() || "Overview";
        setActive(
          labelByTarget.get(text) || (text === "Overview" ? "Dashboard" : text),
        );
      };
                    ...
                    onClick={async () => {
                      ...
                      const buttons = Array.from(
                        document.querySelectorAll<HTMLButtonElement>(
                          ".sidebar > nav:not(.grouped-nav) .nav",
                        ),
                      );
                      const button = buttons.find(
                        (candidate) =>
                          candidate.textContent?.trim() === item.target,
                      );
...
export default function GroupedApp({
  ...
  return (
    <div
      className={
        sessionMode ? "grouped-shell session-overlay-active" : "grouped-shell"
      }
    >
      <App />
      <SidebarBridge sessionMode={sessionMode} />
    </div>
  );
}
